...
async function processPeepMessage(msg: { content: { toString: () => any;}}) {
    ...
    console.log('Saved peep to Query DB');
};

async function processQuerySquawkMessage(msg: { content: { toString: () => any;}; }) {
    const content = JSON.parse(msg.content.toString());
    ...
    try {
        ...
        await newQuery.save();
        console.log("Saved birdsquawk event to Query DB");
    } catch (err) {
        console.log(err);
    }
};
...
const Startup = async () => {
    try {
        ...
        await channel.consume("query-squawk-queue", async (msg: { content: { toString: () => any; }; }) => {
            console.log("Processing message")
            await processQuerySquawkMessage(msg)
            await channel.ack(msg)
        }, { noAck: false });
    ...
    app.listen( 5200, () => {
        console.log('Query-Service is listening on port 5200');
    });
};
